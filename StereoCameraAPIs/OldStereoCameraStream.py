...
class OldStereoCameraStream:
    def __init__(self, cam1=0, cam2=2, framerate=30, resolution=(320, 240), debugEnable=False, debugCount=1000):
        ...
        """
        if not debugEnable:
            self.left = OldMonoLensStream(cam1, framerate, resolution).start()
            self.right = OldMonoLensStream(cam2, framerate, resolution).start()
            self.debugEnable = False
        else:
            self.left = OldMonoLensStream(cam1, framerate, resolution, debugEnable=True, debugCount=debugCount).start()
            self.right = OldMonoLensStream(cam2, framerate, resolution, debugEnable=True, debugCount=debugCount).start()
